...
`listConversionGoals()` calls `GetConversionGoalsByIds` (Campaign Management, `CAMPAIGN_MGMT_BASE` — same base url as `listCampaigns`) through the existing `apiCall()`/`getHeaders()` path. `getConversionPerformance()` calls `ConversionPerformanceReportRequest` through the existing `submitReport()`/`waitForReport()` flow, identical in structure to `getKeywordPerformance`/`getSearchTermReport` — no new request-building path, no new polling logic for either tool.

**`GetConversionGoalsByIds`'s REST body diverges from its own SOAP docs — verified via live smoke test, not discoverable from Microsoft's documentation alone.** Three corrections, none of which any unit test can catch since they're all wire-format issues, not response-mapping issues:
1. **`ConversionGoalIds` must be omitted from the body entirely**, not sent as `[]` or `null`. The SOAP docs say null/empty means "all goals"; the REST binder treats `[]` as null and 400s with `NullArrayArgument`.
2. **`ConversionGoalTypes` must be a comma-delimited string** (`"AppDownload,AppInstall,..."`), not the space-delimited string the SOAP docs literally show, not a JSON array, and not `null`. A space-delimited string or an array both 400 with a generic `NullRequest`/"the request message is null" that gives no hint the actual problem is the delimiter.
3. **`InStoreTransaction` is dropped from the default "all types" list** — this manager customer isn't enrolled in Microsoft's In-Store Transaction pilot, so requesting it 400s with `InStoreTransactionPilotNotEnabledForCustomer` for every account under this manager. If Pathfinder ever onboards a customer that IS pilot-enrolled, this becomes a per-customer decision, not a global one.

If you touch this method, re-run `scripts/smoke-test-conversion-tracking.js` against a real account before considering it done — the failure modes above only surface against Microsoft's live API, never in `npm test`.
...
**Breaking change, not versioned/fallback-gated.** The three existing reporting tools (`bing_ads_get_campaign_performance`, `bing_ads_keyword_performance`, `bing_ads_search_term_report`) requested Microsoft's deprecated `Conversions` column; they now request `ConversionsQualified` instead. **The JSON key itself changes, not just the values** — `parseCsv()` uses the requested column name verbatim as the response's JSON key (confirmed via live smoke test against production, 2026-07-13), so callers see a `ConversionsQualified` field where they previously saw `Conversions`, and the values are not directly comparable to pre-change values either. This ships as a single breaking change with no parallel `use_qualified_conversions`-style parameter — if a real caller needs a migration path later, that's a separate decision made when the need is real, not built speculatively.
