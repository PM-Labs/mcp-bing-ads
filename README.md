...
- `bing_ads_get_campaign_performance(start_date, end_date)` -- Campaign metrics (returns `ConversionsQualified`, not `Conversions` -- Microsoft's qualified/credited decimal column)
...
### Keywords
- `bing_ads_keyword_performance(start_date, end_date, [campaign_ids])` -- Keyword metrics & QS (returns `ConversionsQualified`, not `Conversions` -- Microsoft's qualified/credited decimal column)
- `bing_ads_search_term_report(start_date, end_date)` -- Search terms that triggered ads (returns `ConversionsQualified`, not `Conversions` -- Microsoft's qualified/credited decimal column)
